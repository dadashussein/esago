...
export default function Sidebar({ children, auth, logOut }) {
    ...
    return (
        <aside className="h-screen">
            <nav className="h-full flex flex-col bg-white border-r dark:border-[#686D76]
            shadow-sm dark:bg-darkColor-2">
                <div className="p-4 pb-2 flex justify-between items-center">
                    <p className={`overflow-hidden dark:text-gray-400 transition-all ${expanded ? "w-32" : "w-0"
                        }`}>Esago</p>
                    <button
                ...
                <div className="border-t dark:border-[#686D76] flex p-3 relative">
                    <img
                        src={auth?.profile_picture || "https://ui-avatars.com/api/?name=John+Doe"}
                        alt=""
          ...
          `}
                    >
                        <div className="leading-4">
                            <h4 className="font-semibold dark:text-gray-400">{auth?.username}</h4>
                            <span className="text-xs text-gray-600 dark:text-gray-400">{auth?.email}</span>
                        </div>
                        {dropdown && (
